...
return SelectedUser ? (
    <div className="h-full overflow-scroll relative backdrop-blur-lg">
      ...
      <div className="flex items-center gap-3 py-3 mx-4 border border-stone-500">
        ...
        />
        <p className="flex-1 text-lg cursor-pointer text-white flex items-center gap-2"
        onClick={()=>{
           setShowSideBar(true);
        }}
        >
      ...
      <div className="flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6">
        {messages.map((message:any, index:number) => {
          return (
            ...
            >
              {message.image ? (
                <img src={message.image} alt="" className="w-80 rounded-2xl p-2 border-lg cursor-pointer" 
